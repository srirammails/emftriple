...
import com.emftriple.resource.ETripleObject.EObjectFactory;
...
public class GetProxyObjectImpl extends AbstractGetObject implements GetObject {
	...
	public <T> T get(Class<T> entityClass, URI key) {
		...
		final EClass aClass = Functions.transform(key, 
				new TypeResolver(entityClass, mapping, aGraph));
	...
	private Object createProxy(Node node, EReference reference) {
		Object proxy = null;
		if (dataSourceManager.containsKey(getURI(node))) {
			proxy = dataSourceManager.get(node);
		}
	...
	private Object createProxy(EClass nodeClass, URI key) {
		...
		return EObjectFactory.createProxy(resource, nodeClass, key);
	}
