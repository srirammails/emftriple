 ...
 * @since 0.6.0
 */
public class JpqlSelect2SparqlSelect implements Function<SelectStatement, SelectQuery> {
	...
	private Variable transform(SelectExpression entry) {
		...
		throw new IllegalArgumentException(entry.getClass().getName() + " are not yet suppported");
	}
	...
	private List<GraphPattern> transformFromClass(FromClass from) {
		...
		list.setObject( createIRI( EntityUtil.getRdfTypes(eClass).get(0).toString() ));
	...
	protected static IRI createIRI(EStructuralFeature feature) {
		final IRI iri = factory.createIRI();
		iri.setValue( EntityUtil.getRdfType(feature).toString() );
	...
	private static class WhereSwitch extends JpqlSwitch<List<GraphPattern>> {
		...
		public WhereSwitch(JpqlSelect2SparqlSelect trans) {
			this.trans = trans;
		}
