...
public class FileDescriptorModule extends ETripleModule {
	...
	private static PersistenceMetaData getConfigFromFile() {
	...
	private static PersistenceMetaData getConfiguration(Resource resource) {
		checkIsPersistence(resource);
		
		return (PersistenceMetaData) resource.getContents().get(0);
	}

	private static void checkIsPersistence(Resource resource) {
		checkNotNull(resource);
		checkArgument(resource.getContents().size() == 1);
		checkArgument(resource.getContents().get(0) instanceof PersistenceMetaData);
	}
